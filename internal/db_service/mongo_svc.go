...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
			svc.ServerHost = enviro("FPJP_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
			port := enviro("FPJP_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
			svc.UserName = enviro("FPJP_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
			svc.Password = enviro("FPJP_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
			svc.DbName = enviro("FPJP_API_MONGODB_DATABASE", "fpjp-ambulance")
	}
	...
	if svc.Timeout == 0 {
			seconds := enviro("FPJP_API_MONGODB_TIMEOUT_SECONDS", "10")
